...
$Form1 = GUICreate("停电监控", 380, 90, 265, 254)
$lb1 = GUICtrlCreateLabel("Label2", 8, 8, 280, 45)
$lb2 = GUICtrlCreateLabel("按ESC" & " / " & "终止关机", 280, 5, 100, 45)
...
GUISetState(@SW_SHOW) ;@SW_SHOW显示主窗口，@SW_HIDE 不显示 隐藏
TrayTip("正在运行","断电监控",0,$TIP_ICONASTERISK)
;while 1 会阻止后面的while 执行
monitor()
   ...
   While $i < $cd ; $i <15s -> recheck else  shutdown
	  isConnect3()
		If isConnect1() Then
			...
			GUICtrlSetData($lb1,"运行正常" )
			GUICtrlSetColor($lb1, 0x008000);绿色
			GUICtrlSetData($pgs1, 0)
		Else
		   $i = $i + 1
		   GUISetState(@SW_SHOW) ;show GUI
		   GUICtrlSetData($lb1,"断电! 关机中 "& ($cd-$i) &"秒" )
		   GUICtrlSetColor($lb1, 0xFF0000);红色
